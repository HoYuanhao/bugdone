...
function bindUserInfo(datas) {
	...
	if (typeof (i) != "undefined") {
		...
		jQuery.cookie('bugdone_tabchange', i, {
			expires : 100
		});
	}
...
var closechatlog;
function getchat() {
	jQuery.cookie('bugdone_havechat', 0, {
		expires : 1
	});
		...
		onReceived : function(message) {
			...
			case RongIMClient.MessageType.TextMessage:
				if (message.senderUserId == "system_10000") {
					...
					$(".icon-chat1").attr("title", "你有新消息").css("color", "green");
					$(".icon-chat1").parent(".f-r").addClass("animated tada");
					jQuery.cookie('bugdone_havechat', 1, {
						expires : 7
					});
...
$(".project-logo").mouseover(function() {
	$(".projectHeadImageGray").css("display", "block");
	$("#btnChangePhoto").css("display", "block");
})
$(".project-logo").mouseout(function() {
	$(".projectHeadImageGray").css("display", "none");
	$("#btnChangePhoto").css("display", "none");
})
...
function btnSave_Click() {
	...
	var txtPN = document.getElementById("txtProjectName");
	if (txtPN.value == null || txtPN.value == undefined || txtPN.value == '') {
		return false;
	}
	else
		$("#createInfoBox").show();
...
$(function() {
	...
	$(".main-button").click(function() {
		var regex = /^\w+@\w+.\w+$/; // /^(.+)@(.+)$/
		var email = document.getElementById("txtAddEmail");
		if (!email.value) {
			alert("邮箱地址不能为空！");
			...
			$.ajax({
				type : "get",
				url : "/bugmanage/items/addmember?email=" + $("#txtAddEmail").val() + "&itemsid=" + $('#itemsid').text(),
				async : false,
				data : JSON,
				success : function(data) {
					if (data.result == 1) {
						var html = "<li class=\"li-drag pUserLi\"puserid=\"e179f360-5299-477d-ae5e-d2b12e56a66a\">" +
							"<span class=\"status color-success\">" + data.status + "</span><img src=\"../image/22.png\" ondragstart=\"return false\">" +
							"<p><label id=\"txtCreater\">" + data.username + "</label>" + "<font class=\"color-red\">" +
							" <label id=\"txtAdmin\">" + "普通用户" + "</label></font></p>" +
							"<p><label id=\"txtCreaterEmail\">" + data.email + "</label></p>";
						$("#addMember").hide();
						$(".wrap-grag-bg").hide();
						$("#numberlist").html($("#numberlist").html() + html);
	...
	function additems(){
			...
			$.ajax({
				type : "get",
				url : "/bugmanage/items/newitems?projectname=" + $("#txtProjectName").val() + "&projecttext=" + $("#txtDescription").val() + "&itemsid=" + $('#itemsid').html(),
				async : false,
				data : JSON,
				success : function(datas) {
					...
					if (datas.result == 1) {
						...
						$('#aftersuccess1').show();
						$('#aftersuccess').show();
						$('#txtProjectCreateTime').html(datas.creationtime);
						$('#txtProjectCreateUser').html(datas.creater);
						$('#txtCreater').html(datas.creater);
						$('#txtAdmin').html("拥有者");
						$('#txtCreaterEmail').html(datas.email);
						$('#itemsid').html(datas.itemsid);
						var html = "";
						html += "<li class=\"bs-08 ui-state-default ui-sortable-handle\"projectid=\"6f917cc1f60d4cad85909737e5644eb9\"style=\"position:relative;\">";
						html += "<span style=\"color:white;padding:5px;position:absolute;right:0px;top:0px;background:green;border-radius:2px;\">" + datas.creater;
						html += "的创建</span><div class=\"name\"><a href=\"/ProjectConsole-6f917cc1f60d4cad85909737e5644eb9\"title=\"bug管理系统\">" + $("#txtProjectName").val();
						html += "</a></div><div class=\"image bd-e2 b-radius4\"><a href=\"../items/project1?id=" + $('#itemsid').text() + "&&actionnumber=0&&problemnumber=0\"><img src=\"..";
						html += "/itemspic/2017-10-30/qcit.jpg" + "\" alt=\"bug管理系统\" width=\"100\"align=\"middle\">";
						html += "</a></div><div class=\"info info-1\" style=\"overflow: visible;\"><span class=\"f-l\"> " +
							"<font class=\"f-l color-8\">活动问题</font><i class=\"num n-bg1 f-l\">" + "0" + "</i></span> " +
							"<span class=\"f-l\"> <font class=\"f-l color-8\">我的待办</font> <i class=\"num n-bg2 f-l\">0</i></span></div><div class=\"btn\"><a href=\"javascript:void(0)\" onclick=\"changeSetting('6f917cc1f60d4cad85909737e5644eb9');\">" +
							"<i class=\"iconfont icon-iconshezhi01\"></i>配置</a> <a href=\"javascript:void(0)\" onclick=\"copyProject('6f917cc1f60d4cad85909737e5644eb9');\"><i class=\"iconfont icon-fuzhi\"></i>复制</a>" + " <a href=\"javascript:void(0)\" onclick=\"out('6f917cc1f60d4cad85909737e5644eb9');\">" +
							"<i class=\"iconfont icon-tuichu1\"></i>退出</a> </div></li>";
						$("#ulb").html($("#ulb").html() + html);
						var cpcount=document.getElementById("cpcount").innerHTML;
						var pccount=document.getElementById("pccount").innerHTML;
						
						$("#cpcount").html(Number(cpcount)+1);
						$("#pccount").html(Number(pccount)+1);
						alert("创建项目成功.");
					}
					if (datas.result == 0) {
						alert("更新成功！");
...
$(document).ready(function(){
	var html=$("#tianjia").html(); 
	$("input:hidden[name='closecache']").val(html);
	$("#clear_Click").onclick(function() {
		$("#tianjia").html(html);
		$("#closecache").val(html);
		});
});
...
function clear_Click(){
	var html=$("input:hidden[name='closecache']").val();
	$("#tianjia").html(html);
}