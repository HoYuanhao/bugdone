...
public class UserController {
   ...
   @Autowired
   NumberStatService numberStatService;
	/**
	 * 获取验证码
	 * 
	 * @param request
	...
	String login(@RequestParam("email")String email,
		...
		logger.debug("email:"+email+"登录。");
		return result;

	}
	@RequestMapping(value="getSimpleHandler",produces="application/json;charset=UTF-8")
	public
	@ResponseBody
	String getSimpleHandler(@RequestParam("pid")int pid){
		List<SimpleHandlerVo> list=userService.getSimpleHandlerVo(pid);
		logger.debug("得到用户列表："+list);
	...
	public String index(HttpSession session,HttpServletRequest request) {
		...
		List<ItemsListVo> items=itemsService.getItemsList(user.getId(), "1",request);
		IndexNumberStat indexNumber=numberStatService.getIndexNumberStat(user.getId());
		logger.debug("ItemsListVo:"+items);
		logger.debug("indexNumber:"+indexNumber);
	...
	String register(@RequestParam("email") String email, 
		...
		if (result.equals("1")) {
			jsonObject.put("user", (User) request.getSession().getAttribute(Constant.user.name()));
		}
